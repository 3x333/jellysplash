  ...
  export function drawSplash(
    ...
    const {
      columns, gap, tilt, cornerRadius, aspectRatio, jitter,
      brightness, saturation,
      bgColour, overlayType, overlayStrength, overlayColour,
      seed,
    } = config

    // If aspectRatio is 'source', use average ratio of loaded images
    const effectiveRatio: number = aspectRatio === 'source'
      ? images.reduce((sum, { img }) => sum + img.naturalWidth / img.naturalHeight, 0) / images.length
      : aspectRatio
